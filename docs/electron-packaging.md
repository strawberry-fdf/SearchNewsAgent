...
# 终端 3: 启动统一开发入口（可选，也可直接用浏览器 http://localhost:3000）
cd AgentNews
pnpm dev
```
...
| 命令 | 说明 |
|------|------|
| `pnpm dev` | 交互式选择并启动开发服务（backend/frontend/electron） |
| `pnpm check` | 交互式选择质量检查项（TypeScript/Vitest/Pytest） |
| `pnpm release` | 交互式发布（版本类型 + commit message + tag + push） |
| `pnpm build` | 交互式多选平台构建（mac/win/linux） |
| `pnpm clean` | 清理所有构建产物 |
