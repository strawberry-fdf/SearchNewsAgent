...
- 支持**筛选预设方案**扩展：用户自定义 Prompt 附加到 System Prompt 末尾
...
| 页面 | 组件 | 功能 |
|------|------|------|
| 精选 Feed | `ArticleFeed` (mode=feed) | 已入选文章列表，分类/标签/关键词筛选 |
| 全部文章 | `ArticleFeed` (mode=all) | 所有文章（含被拒绝），状态筛选 |
| 收藏 | `ArticleFeed` (mode=starred) | 用户收藏的文章 |
| 统计 | `StatsPanel` | 文章计数 + Pipeline 执行历史 + SSE 日志 |
| 信源管理 | `SourceManager` | 信源列表增删改，启用/禁用 |
| 设置 | `Settings` | LLM 开关、兴趣标签、关键词规则、预设方案 |

#### 核心组件

- **ArticleCard**: 展示单篇文章卡片，含分数徽章、展开详情、收藏、删除、用户标签编辑
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/articles/selected` | 获取精选文章列表（分页+筛选） |
...
| POST | `/api/filter-presets/{id}/activate` | 激活预设 |
