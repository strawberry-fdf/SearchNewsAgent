#!/usr/bin/env bash
# ============================================================
# 使用 PyInstaller 将 AgentNews 后端打包为 macOS 可执行文件
# ============================================================
#
# 步骤:
#   1. 激活虚拟环境（如果存在）
...
#
# 使用: bash scripts/build-backend-mac.sh
...
# macOS 后端作为 Electron 子进程运行，使用 --console 模式（非 --windowed）
# --windowed 会生成 .app 包结构导致目录拆解后 _internal 路径不对
pyinstaller \
    --name=backend \
    --onedir \
    --console \
...
# --console 模式直接输出到 pyinstaller_dist/backend/ 目录
